...
from model2 import DualNet
import tensorflow.compat.v1 as tf

parser = argparse.ArgumentParser(description='Argument parser')
""" Arguments related to network architecture"""
parser.add_argument('--image_size', dest='image_size', type=int, default=256, help='size of input images (applicable to both B images and C images)')
...
parser.add_argument('--C_channels', dest='C_channels', type=int, default=3, help='# of channels of image C')
...
parser.add_argument('--lambda_C', dest='lambda_C', type=float, default=500.0, help='# weights of C recovery loss')
...
def main(_):
    ...
    with tf.Session() as sess:
        model = DualNet(sess, image_size=args.image_size, batch_size=args.batch_size,\
                        dataset_name=args.dataset_name,B_channels = args.B_channels, \
						C_channels = args.C_channels, flip  = (args.flip == 'True'),\
                        checkpoint_dir=args.checkpoint_dir, sample_dir=args.sample_dir,\
						gcn = args.gcn, dcn=args.dcn, \
						loss_metric=args.loss_metric, lambda_C=args.lambda_C, \
						lambda_B= args.lambda_B, dropout_rate=args.dropout_rate, \
