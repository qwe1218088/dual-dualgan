...
class DualNet(object):
    def __init__(self, sess, image_size=256, batch_size=1, gcn=64, dcn=64, \
                 A_channels=3, B_channels=3, C_channels=3, dataset_name='facades', \
                 checkpoint_dir=None, lambda_A=500., lambda_B=500.,lambda_C=500, \
        ...
        # directory name for output and logs saving 输出文件命名
    ...
    def build_model(self):
        ...
        if self.GAN_type == 'classic':
            self.Ag_loss = celoss(self.Ad_logits_fake,
                                  labels=tf.ones_like(self.Ad_logits_fake)) + self.lambda_B * self.B_loss
            self.Bg_loss = celoss(self.Bd_logits_fake,
                                  labels=tf.ones_like(self.Bd_logits_fake)) + self.lambda_C * self.C_loss
        else:
    ...
    def load_random_samples(self):
        ...
        sample_B_imgs = np.reshape(np.array(sample_B_imgs).astype(np.float32),
                                   (self.batch_size, self.image_size, self.image_size, -1))
    ...
    def sample_shotcut(self, sample_dir, epoch_idx, batch_idx):
        ...
        Ag, A2B2A_imgs, A2B_imgs = self.sess.run([self.A_loss, self.A2B2A, self.A2B],
                                                 feed_dict={self.real_A: sample_A_imgs, self.real_B: sample_B_imgs})
        Bg, B2A2B_imgs, B2A_imgs = self.sess.run([self.B_loss, self.B2A2B, self.B2A],
                                                 feed_dict={self.real_A: sample_A_imgs, self.real_B: sample_B_imgs})

        save_images(A2B_imgs, [self.batch_size, 1],
                    './{}/{}/{:06d}_{:04d}_A2B.jpg'.format(sample_dir, self.dir_name, epoch_idx, batch_idx))
        save_images(A2B2A_imgs, [self.batch_size, 1],
                    './{}/{}/{:06d}_{:04d}_A2B2A.jpg'.format(sample_dir, self.dir_name, epoch_idx, batch_idx))

        save_images(B2A_imgs, [self.batch_size, 1],
                    './{}/{}/{:06d}_{:04d}_B2A.jpg'.format(sample_dir, self.dir_name, epoch_idx, batch_idx))
        save_images(B2A2B_imgs, [self.batch_size, 1],
    ...
    def train(self, args):
        ...
        if self.GAN_type == 'wgan':
            self.clip_ops = [var.assign(tf.clip_by_value(var, -self.clip_value, self.clip_value)) for var in
                             self.d_vars]
        ...
        for epoch_idx in range(args.epoch):
            ...
            np.random.shuffle(data_A)  # 随机选择吗？
    ...
    def fcn(self, imgs, prefix=None, reuse=False):
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            self.d1, self.d1_w, self.d1_b = deconv2d(tf.nn.relu(e8),
                                                     [self.batch_size, s128, s128, self.gcn * 8], name=prefix + 'd1',
                                                     with_w=True)
            ...
            self.d2, self.d2_w, self.d2_b = deconv2d(tf.nn.relu(d1),
                                                     [self.batch_size, s64, s64, self.gcn * 8], name=prefix + 'd2',
                                                     with_w=True)
            if self.dropout_rate <= 0.:
            ...
            self.d3, self.d3_w, self.d3_b = deconv2d(tf.nn.relu(d2),
                                                     [self.batch_size, s32, s32, self.gcn * 8], name=prefix + 'd3',
                                                     with_w=True)
            if self.dropout_rate <= 0.:
            ...
            self.d4, self.d4_w, self.d4_b = deconv2d(tf.nn.relu(d3),
                                                     [self.batch_size, s16, s16, self.gcn * 8], name=prefix + 'd4',
                                                     with_w=True)
            ...
            self.d5, self.d5_w, self.d5_b = deconv2d(tf.nn.relu(d4),
                                                     [self.batch_size, s8, s8, self.gcn * 4], name=prefix + 'd5',
                                                     with_w=True)
            ...
            self.d6, self.d6_w, self.d6_b = deconv2d(tf.nn.relu(d5),
                                                     [self.batch_size, s4, s4, self.gcn * 2], name=prefix + 'd6',
                                                     with_w=True)
            ...
            self.d7, self.d7_w, self.d7_b = deconv2d(tf.nn.relu(d6),
                                                     [self.batch_size, s2, s2, self.gcn], name=prefix + 'd7',
                                                     with_w=True)
            ...
            if prefix == 'B_g_':
                self.d8, self.d8_w, self.d8_b = deconv2d(tf.nn.relu(d7), [self.batch_size, s, s, self.A_channels],
                                                         k_h=5, k_w=5, name=prefix + 'd8', with_w=True)
            elif prefix == 'A_g_':
